...
### Added
- **Vercel one-click deploy button** — `README.md`, `docs/QUICKSTART_EN.md`, and `docs/QUICKSTART_DE.md` now include a `Deploy with Vercel` button that lets anyone spin up their own hosted instance in under a minute without any local tooling.
- **`agents.md` enhanced** — Added *Build, Test & Lint Commands* reference table and *Repository Conventions* section so AI agents have all critical codebase facts in a single file.
...
### Fixed
- **PDF/Excel digital split export now uses per-source after-fee buckets.** The PDF waterfall no longer applies the Believe fallback percentage to the entire digital total. Digital artist share is now calculated source-aware (`Believe`, `Bandcamp`, `Other`) from after-fee bucket values, matching `finalPayout`. The waterfall split label now stays compact while still showing source-specific deviations (only for sources with revenue), and Excel Summary now lists separate digital split lines for Believe/Bandcamp/Other when relevant.
- **Believe and Bandcamp revenue now use separate split percentages.** Previously, `sourceSplits.believe` and `sourceSplits.bandcamp` were collapsed into a single digital bucket split using `??`, so Believe's rate was applied to all digital revenue regardless of source. Now each source has its own independent bucket: Believe revenue uses `sourceSplits.believe`, Bandcamp revenue uses `sourceSplits.bandcamp`, and all other digital sources use the standard digital chain. Per-artist `sourceOverrides` for `believe`/`bandcamp` retain highest priority and work independently per source.
- **Physical bucket split now respects per-artist `physicalPercentage` overrides.** When an artist has an explicit **Physical %** configured in Split Fees (Blackbook), it takes priority over the General Settings Physical Split % (Source Split Rates → Physical). Previously, an active bucket split bypassed artist-level physical settings entirely, causing the per-artist Physical % to have no effect. The new priority order for the physical bucket is: (1) per-artist source override (shopify/printful), (2) per-artist `physicalPercentage`, (3) General Settings Physical Split %, (4) main chain fallback.
- **Track revenue assignments: zero-percentage co-owners no longer appear in statements.** Artists with `percentage: 0` (or an empty artist name) in a `TrackRevenueAssignment` are now filtered out before any transaction is cloned. Previously, zero-percentage co-owners received a €0.00 transaction clone that caused phantom entries in the Statement of Sales and polluted revenue calculations.
